...
// Receives article from BabyLoveGrowth, generates HTML, commits to GitHub → Netlify auto-deploys
//
// Required Netlify Environment Variables:
//   GITHUB_TOKEN       – GitHub Personal Access Token (repo scope)
//   GITHUB_REPO        – e.g. "alexios-donas/planegy" (owner/repo)
...
// Category → emoji + gradient mapping
const CAT_STYLE = {
...
exports.handler = async (event) => {
  ...
  const readTime = estimateReadTime(content_html);
  const dateStr = published_at || new Date().toISOString();

  // Generate the plain-text excerpt from HTML
  ...
  const articleHtml = generateArticleHtml({ title, slug, content_html, metaDescription: excerpt, category, catStyle, readTime, dateStr });

  // 1. Create/update the article HTML file
  const existingArticle = await getFile(articleFileName);
  const articleResult = await putFile(
    articleFileName,
    articleHtml,
  ...
  if (articleResult.status !== 200 && articleResult.status !== 201) {
    console.error("GitHub error creating article:", JSON.stringify(articleResult.data));
    return { statusCode: 500, body: JSON.stringify({ error: "Failed to save article to GitHub", detail: articleResult.data }) };
  }

  // 2. Update blog.html — inject new article card after BLOG_ARTICLES_START
  ...
  if (blogFile) {
    ...
    if (blogFile.content.includes(marker)) {
      const updatedBlog = blogFile.content.replace(marker, marker + newCard);
      await putFile("blog.html", updatedBlog, `blog: add card for "${title}"`, blogFile.sha);
    }
  }

  return {
    statusCode: 200,
    body: JSON.stringify({ success: true, file: articleFileName, slug }),
  };
};
